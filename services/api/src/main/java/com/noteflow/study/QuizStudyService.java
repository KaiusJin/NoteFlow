...
public class QuizStudyService {
    ...
    private void attempt(UUID id, UUID userId, String status) {
        List<Object> args = new ArrayList<>(List.of(id, userId));
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM quiz_attempts WHERE id=? AND user_id=?");
        if (status != null) {
            sql.append(" AND status=?");
            args.add(status);
        }
        Integer count = jdbc.queryForObject(sql.toString(), Integer.class, args.toArray());
        if (count == null || count == 0) throw new IllegalArgumentException("Attempt not found or invalid state");
    }
