...
class GenerateFlashcardsPipeline:
    ...
    def run(self, payload: TaskPayload) -> None:
        ...
        try:
            ...
            options = self.repo.load_flashcard_generation_options(deck_id)
            ...
            groups = build_source_groups(
                chunks, settings.flashcards_group_target_tokens, settings.flashcards_group_max_tokens,
                respect_sections=options.get("groupBySection") is not False,
            )
            targets = allocate_item_targets(groups, settings.flashcards_per_1000_source_tokens,
                                            settings.flashcards_max_per_document)
            requested_count = options.get("targetCount")
            if isinstance(requested_count, int) and not isinstance(requested_count, bool) and requested_count > 0:
                allocation = allocate_difficulty_targets(groups, {"cards": requested_count})
                targets = {index: mix["cards"] for index, mix in allocation.items()}
            ...
            failed: dict[int, str] = {}
            pending = [group for group in groups if group.index in targets and group.index not in completed]
            with ThreadPoolExecutor(max_workers=max(1, settings.flashcards_max_concurrent_requests)) as executor:
                futures = {executor.submit(generate_group, provider, prompt(source_title, group, len(targets), targets[group.index], focus),
            ...
            try:
                LearningMemoryRepository().sync_artifact_topics("FLASHCARDS", deck_id, payload.user_id)
            except Exception as exc:
                report["learningMemorySyncWarning"] = str(exc)[:500]
                self.repo.update_generation("flashcard_decks", deck_id, "READY", provider.provider_name, provider.model,
                                            PROMPT_VERSION, len(groups), len(completed), report)
