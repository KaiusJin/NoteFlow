...
from noteflow_worker.pdf.regions import analyze_regions_with_vlm, build_visual_regions
...
class ParseDocumentPipeline:
    ...
    def run(self, payload: TaskPayload) -> None:
        try:
            ...
            if parsed.content_source_type in {"SCANNED_PDF", "HANDWRITTEN_SCAN"}:
                import json
                from noteflow_worker.db.repository import VisualRegion, LayoutBlock
                from noteflow_worker.pdf.layout import WorkingBlock, build_structural_chunks
                ...
                for v_page in visual_pages:
                    visual_regions.append(
                        VisualRegion(
                            ...
                            region_index=0,
                            region_type="HANDWRITTEN" if parsed.content_source_type == "HANDWRITTEN_SCAN" else "FULL_PAGE_VISUAL",
                ...
                vlm_results = analyze_regions_with_vlm(visual_regions, fail_on_error=True)
                ...
                self._repository.mark_processing(payload.task_id, payload.document_id, "VLM_ANALYSIS", 65)
                vlm_results = analyze_regions_with_vlm(visual_regions)
                self._repository.replace_vlm_results(payload.document_id, vlm_results)
            ...
            parser_name = "gemini-page-level-vlm" if parsed.content_source_type in {"SCANNED_PDF", "HANDWRITTEN_SCAN"} else "pymupdf-layout-aware+vlm-assets"
